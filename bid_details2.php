  ...
  <div class="container">
    ...
    <section>
      <div class="details_container">
        ...
        <!-- <a href="https://chat.gise.at/#apvuqpcrqvaokjavdhbf">Chat</a><br><br><br><br><br><br> -->
        ...
        if ($check_empty) {
          while ($check_empty = mysqli_fetch_assoc($query_run)) {
            if(isset($_POST['accept'])){
              $_SESSION['clientusername'];
              $sql1 = "UPDATE `projectbids` SET chahiye = 'ACCEPT' WHERE projectidofbids = '16'" ;
              $sqlrun = mysqli_query($conn, $sql1);
              echo "$projectidofbids";
              if(sqlrun)
              {
                echo "<script>alert('Updated as Accepted')</script>";
              }
              else{
                echo "<script>alert('Neet chutiya')</script>";
              }
            }
            // if(isset($_POST['reject'])){
            //   $sql11 = "UPDATE `projectbids` SET chahiye = 'REJECT' WHERE projectidofbids = `$projectidofbids` " ;
            //   $sqlrun1 = mysqli_query($conn, $sql11);
            //   if(sqlrun1)
            //   {
            //     echo "<script>alert('Updated as Rejected')</script>";
            //   }
            // }
